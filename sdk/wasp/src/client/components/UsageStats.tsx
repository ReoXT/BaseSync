...
function StatRow({ stat, warningThreshold, compact = false }: StatRowProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      {!isUnlimited && (
        <>
          <Progress
            ...
            indicatorClassName={getProgressColor(percentage, warningThreshold)}
          />
      ...
      {isUnlimited && (
        <p className="text-muted-foreground text-xs">No limit on your plan</p>
      )}
    </div>
  );
}
...
export function UsageStats({
  ...
  const hasWarning = allStats.some(
    (stat) => getUsagePercentage(stat.current, stat.limit) >= warningThreshold
  );

  return (
    <Card className={cn(hasWarning && "border-yellow-500")}>
...
export function CompactUsageStats(props: Omit<UsageStatsProps, "compact">) {
