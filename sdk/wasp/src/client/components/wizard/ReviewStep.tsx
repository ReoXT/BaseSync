...
export function ReviewStep({ formData, isEditMode = false, syncConfigId }: ReviewStepProps) {
  ...
  const handleCreateSync = async () => {
    ...
    try {
      if (!formData.syncName) throw new Error("Sync name is required");
      ...
      if (!formData.syncDirection) throw new Error("Sync direction is required");
  ...
  return (
    <div className="space-y-6">
      ...
      <Button
        ...
        className="w-full h-12 bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white shadow-lg shadow-cyan-500/20 disabled:opacity-50"
      >
        {isCreating ? (
          <>
