...
import { getPaymentPlanIdByPaymentProcessorPlanId, SubscriptionStatus, } from "../plans";
...
async function handleCustomerSubscriptionUpdated(event, prismaUserDelegate) {
    ...
    const paymentPlanId = getPaymentPlanIdByPaymentProcessorPlanId(getSubscriptionPriceId(subscription));
    const user = await updateUserSubscription({ paymentProcessorUserId: customerId, paymentPlanId, subscriptionStatus }, prismaUserDelegate);
...
function getSubscriptionPriceId(subscription) {
    ...
    if (subscriptionItems.length !== 1) {
        throw new Error("There should be exactly one subscription item in Stripe subscription");
    }
