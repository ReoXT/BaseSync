 ...
 */
export const listUserAirtableBases = async (_args, context) => {
    if (!context.user) {
    ...
    if (!connection) {
        throw new Error('Airtable account not connected. Please connect your Airtable account first.');
    }
    try {
        // Get access token (automatically refreshes if expired)
        const accessToken = await getAirtableAccessToken(context.user.id, context.entities.AirtableConnection);
        ...
        if (error instanceof Error) {
            if (error.message.includes('refresh') || error.message.includes('token')) {
                throw new Error('Your Airtable connection has expired. Please reconnect your Airtable account.');
            }
 ...
 * Automatically refreshes token if expired
 */
export const getAirtableTableSchema = async (args, context) => {
    if (!context.user) {
        throw new Error('User must be authenticated');
    }
 ...
 * Automatically refreshes token if expired
 */
export const getAirtableBaseTables = async (args, context) => {
    if (!context.user) {
        throw new Error('User must be authenticated');
    }
    const { baseId } = args;
