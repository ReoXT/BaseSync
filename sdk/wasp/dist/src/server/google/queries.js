 ...
 */
export const validateSpreadsheetUrl = async (args, context) => {
    if (!context.user) {
    ...
    try {
        ...
        const accessToken = await getGoogleSheetsAccessToken(context.user.id, context.entities.GoogleSheetsConnection);
        // Validate spreadsheet exists and user has access
        const spreadsheet = await googleClient.validateAndGetSpreadsheet(accessToken, parsed.spreadsheetId);
        return {
 ...
 */
export const getSpreadsheetSheets = async (args, context) => {
    if (!context.user) {
        throw new Error('User must be authenticated');
    }
    ...
    try {
        ...
        if (error instanceof googleClient.GoogleSheetsError) {
            if (error.isAuthError) {
                throw new Error('Your Google connection has expired or lacks permissions. Please reconnect your Google account.');
            }
            if (error.isQuotaError) {
                throw new Error('Google API quota exceeded. Please try again later or contact support.');
            }
            if (error.statusCode === 404) {
                throw new Error(`Spreadsheet with ID "${spreadsheetId}" not found or you don't have access to it.`);
            }
 ...
 * Automatically refreshes token if expired
 */
export const getSheetColumnHeaders = async (args, context) => {
    if (!context.user) {
        throw new Error('User must be authenticated');
    }
    ...
    try {
        ...
        const sheetData = await googleClient.getSheetData(accessToken, spreadsheetId, sheetId, 'A1:ZZ1' // Get first row, up to column ZZ (702 columns)
        );
