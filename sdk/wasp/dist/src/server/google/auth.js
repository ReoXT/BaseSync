...
function getGoogleConfig() {
    ...
    if (!clientId || !clientSecret || !redirectUri) {
        throw new Error('Missing Google OAuth configuration. Please set GOOGLE_SHEETS_CLIENT_ID, ' +
 ...
 */
export async function storeGoogleSheetsConnection(userId, tokenResponse, prisma, googleAccountEmail) {
 ...
 */
export async function getGoogleSheetsAccessToken(userId, prisma) {
    // Get the delegate (either it IS the delegate, or get it from the client)
    const delegate = 'googleSheetsConnection' in prisma
        ? prisma.googleSheetsConnection
        : prisma;
    ...
    if (connection.tokenExpiry && connection.tokenExpiry <= expiryBuffer) {
        ...
        try {
            ...
            await storeGoogleSheetsConnection(userId, newTokens, delegate, connection.googleAccountEmail || undefined);
...
export async function getGoogleUserInfo(accessToken) {
    ...
    if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(`Failed to fetch Google user info: ${response.status} ${response.statusText}. ` +
            `Error: ${JSON.stringify(errorData)}`);
    }
    return await response.json();
}
 ...
 * @param prisma - Prisma client instance
 */
export async function revokeGoogleSheetsConnection(userId, prisma) {
    // Get the delegate (either it IS the delegate, or get it from the client)
    const delegate = 'googleSheetsConnection' in prisma
        ? prisma.googleSheetsConnection
        : prisma;
 ...
 */
export async function hasGoogleSheetsConnection(userId, prisma) {
    // Get the delegate (either it IS the delegate, or get it from the client)
    const delegate = 'googleSheetsConnection' in prisma
        ? prisma.googleSheetsConnection
        : prisma;
