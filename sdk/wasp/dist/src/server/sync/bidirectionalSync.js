...
import { getSheetData, updateSheetData, appendRows, hideColumn, ensureColumnsExist, columnNumberToLetter, } from '../google/client';
...
import { detectConflicts, resolveConflicts, updateSyncState, summarizeConflicts, } from './conflictDetector';
// ============================================================================
...
export async function syncBidirectional(options) {
    ...
    // Set defaults
    const { syncConfigId, airtableAccessToken, sheetsAccessToken, baseId, tableId, viewId, // CRITICAL: Extract viewId for Airtable record ordering
    spreadsheetId, sheetId, conflictResolution, fieldMappings, idColumnIndex = 0, syncTimestampColumnIndex, includeHeader = true, resolveLinkedRecords: shouldResolveLinkedRecords = true, createMissingLinkedRecords = false, maxRetries = 3, batchSize = 10, dryRun = false, } = options;
    const checkpoint = {
        airtableRecords: [],
        sheetsRows: [],
        tableFields: [],
    ...
    try {
        ...
        const fetchPhase = await executePhase('fetch', async () => {
            ...
            checkpoint.airtableRecords = await retryWithBackoff(() => listRecords(airtableAccessToken, baseId, tableId, listRecordsOptions), maxRetries, 'fetch Airtable records');
            ...
            if (fieldMappings && Object.keys(fieldMappings).length > 0) {
                ...
                checkpoint.tableFields = checkpoint.tableFields.filter((f) => mappedFieldIds.includes(f.id));
                checkpoint.tableFields.sort((a, b) => (fieldMappings[a.id] || 0) - (fieldMappings[b.id] || 0));
            }
            ...
            if (shouldResolveLinkedRecords && checkpoint.airtableRecords.length > 0) {
                ...
                const resolved = await resolveAllLinkedRecords(airtableAccessToken, baseId, tableId, checkpoint.airtableRecords, { strictMode: false });
            ...
            const sheetData = await retryWithBackoff(() => getSheetData(sheetsAccessToken, spreadsheetId, sheetId), maxRetries, 'fetch Sheets data');
            ...
            if (idColumnIndex === 0) {
                ...
                console.log(`[Phase 1: Fetch] Using fixed column ${columnLetter} (index ${actualIdColumnIndex}) for record IDs (will be hidden)`);
            }
        ...
        const conflictDetectionPhase = await executePhase('conflictDetection', async () => {
            ...
            checkpoint.conflictResults = detectConflicts(checkpoint.airtableRecords, checkpoint.sheetsRows, syncConfigId, actualIdColumnIndex);
            ...
            if (checkpoint.conflictResults.conflicts.length > 0) {
                console.log(`[Phase 2: Conflict Detection] ⚠️  ${checkpoint.conflictResults.conflicts.length} conflicts detected`);
            }
            return {
                metadata: {
        ...
        if (checkpoint.conflictResults && checkpoint.conflictResults.conflicts.length > 0) {
            const conflictResolutionPhase = await executePhase('conflictResolution', async () => {
                ...
                checkpoint.conflictResolutions = resolveConflicts(checkpoint.conflictResults.conflicts, conflictResolution);
                ...
                checkpoint.conflictResolutions.forEach((resolution) => {
                    ...
                    console.log(`[Phase 3: Conflict Resolution]   ${resolution.recordId}: ${resolution.action} (${resolution.reason})`);
                });
        ...
        const airtableToSheetsPhase = await executePhase('airtableToSheets', async () => {
            ...
            for (const record of recordsToUpdate) {
                try {
                    ...
                    const existingRowIndex = checkpoint.sheetsRows.findIndex((r) => r[actualIdColumnIndex] === record.id);
                    if (existingRowIndex >= 0) {
                        // Update existing row with just the data (no record ID in the array)
                        const rowNumber = existingRowIndex + (includeHeader ? 2 : 1);
                        await retryWithBackoff(() => updateSheetData(sheetsAccessToken, spreadsheetId, sheetId, `A${rowNumber}`, [
                            row,
                        ]), maxRetries, `update row ${rowNumber}`, { recordId: record.id, phase: 'airtableToSheets' });
                        updated++;
                        rowsNeedingRecordIds.push({ rowNumber, recordId: record.id });
                    }
                    else {
                        // Append new row with just the data (no record ID in the array)
                        await retryWithBackoff(() => appendRows(sheetsAccessToken, spreadsheetId, sheetId, [row]), maxRetries, `append row for ${record.id}`, { recordId: record.id, phase: 'airtableToSheets' });
                        added++;
                    ...
                    console.error(`[Phase 4: Airtable → Sheets] ✗ Failed to sync record ${record.id}: ${syncError.message}`);
            ...
            if (totalFailed > 0) {
                console.log(`[Phase 4: Airtable → Sheets] ⚠️  Synced ${totalSuccessful}/${totalAttempted} records (${totalFailed} failed)`);
            }
            else {
                console.log(`[Phase 4: Airtable → Sheets] ✓ Synced ${totalSuccessful} records (${added} added, ${updated} updated)`);
            }
            ...
            if ((added > 0 || updated > 0) && !dryRun) {
                try {
                    ...
                    console.log(`[Phase 4: Airtable → Sheets] Ensuring column ${columnLetter} exists (need ${requiredColumnCount} columns)...`);
                    await ensureColumnsExist(sheetsAccessToken, spreadsheetId, sheetId, requiredColumnCount);
                    // STEP 2: Write record IDs to column AA
                    console.log(`[Phase 4: Airtable → Sheets] Writing ${rowsNeedingRecordIds.length} record IDs to column ${columnLetter}...`);
                    for (const { rowNumber, recordId } of rowsNeedingRecordIds) {
                        ...
                        try {
                            ...
                            console.warn(`[Phase 4: Airtable → Sheets] Failed to write record ID to ${range}:`, error);
                        }
                    }
                    ...
                    console.log(`[Phase 4: Airtable → Sheets] ✓ Hidden column ${columnLetter} (users won't see record IDs)`);
                }
                catch (error) {
        ...
        const sheetsToAirtablePhase = await executePhase('sheetsToAirtable', async () => {
            ...
            if (checkpoint.conflictResolutions) {
                checkpoint.conflictResolutions.forEach((resolution) => {
                    if (resolution.action === 'USE_SHEETS') {
                        const rowIndex = checkpoint.sheetsRows.findIndex((r) => r[actualIdColumnIndex] === resolution.recordId);
                        if (rowIndex >= 0) {
                            rowsToSync.add(rowIndex);
                        }
                    }
            ...
            for (const batchIndices of batchedRows) {
                    ...
                    try {
                        ...
                        if (!matchedRecord && !recordId && checkpoint.primaryFieldName && fields[checkpoint.primaryFieldName]) {
                            ...
                            if (primaryValue) {
                                ...
                                if (matchedRecord) {
                                    console.log(`[Phase 5: Sheets → Airtable] Matched row ${rowIndex} to existing record ${matchedRecord.id} by primary field "${checkpoint.primaryFieldName}"`);
                                }
                            }
                        }
                        if (matchedRecord) {
                        ...
                        phaseErrors.push(syncError);
                        console.error(`[Phase 5: Sheets → Airtable] ✗ Failed to transform row ${rowIndex}: ${syncError.message}`);
                    }
                }
                // Execute batch operations with error handling
                if (recordsToCreate.length > 0) {
                    try {
                        const created = await retryWithBackoff(() => createRecords(airtableAccessToken, baseId, tableId, recordsToCreate), maxRetries, 'create records batch', { phase: 'sheetsToAirtable' });
                        ...
                        phaseErrors.push(syncError);
                        console.error(`[Phase 5: Sheets → Airtable] ✗ Failed to create ${recordsToCreate.length} records: ${syncError.message}`);
                        // If it's an OAuth error, stop the entire sync
                        if (syncError.type === 'OAUTH') {
                            throw error;
                        }
                ...
                if (recordsToUpdate.length > 0) {
                    try {
                        const updated_batch = await retryWithBackoff(() => updateRecords(airtableAccessToken, baseId, tableId, recordsToUpdate), maxRetries, 'update records batch', { phase: 'sheetsToAirtable' });
                        ...
                        const syncError = classifyError(error, 'sheetsToAirtable');
                        phaseErrors.push(syncError);
                        console.error(`[Phase 5: Sheets → Airtable] ✗ Failed to update ${recordsToUpdate.length} records: ${syncError.message}`);
                        // If it's an OAuth error, stop the entire sync
                        if (syncError.type === 'OAUTH') {
                            throw error;
                        }
            ...
            const totalFailed = phaseErrors.length;
            if (totalFailed > 0) {
                console.log(`[Phase 5: Sheets → Airtable] ⚠️  Synced ${totalSuccessful}/${totalAttempted} records (${totalFailed} failed)`);
            }
            else {
                console.log(`[Phase 5: Sheets → Airtable] ✓ Synced ${totalSuccessful} records (${added} added, ${updated} updated)`);
            }
            // Write new record IDs back to Google Sheets to prevent duplicates on next sync
            if (newRecordIdUpdates.length > 0 && !dryRun) {
                console.log(`[Phase 5: Sheets → Airtable] Writing ${newRecordIdUpdates.length} new record IDs back to Sheets...`);
                try {
                    const columnLetter = columnNumberToLetter(actualIdColumnIndex + 1);
                    // STEP 1: Ensure column AA exists before writing to it
                    // This prevents errors when the sheet only has columns A-Z (0-25)
                    const requiredColumnCount = actualIdColumnIndex + 1; // Need at least 27 columns for column AA (index 26)
                    console.log(`[Phase 5: Sheets → Airtable] Ensuring column ${columnLetter} exists (need ${requiredColumnCount} columns)...`);
                    await ensureColumnsExist(sheetsAccessToken, spreadsheetId, sheetId, requiredColumnCount);
                    // STEP 2: Write record IDs to column AA
                    // Sort by row number
                    ...
                    for (const update of newRecordIdUpdates) {
                        ...
                        updatePromises.push(updateSheetData(sheetsAccessToken, spreadsheetId, sheetId, range, [[update.recordId]]).catch((error) => {
                            console.warn(`[Phase 5: Sheets → Airtable] Failed to write ID for row ${update.row}:`, error);
                        }));
                    ...
                    console.log(`[Phase 5: Sheets → Airtable] ✓ Successfully wrote ${newRecordIdUpdates.length} record IDs to column ${columnLetter}`);
                    // STEP 3: Hide column AA to keep sheets clean for users
                    try {
                        await hideColumn(sheetsAccessToken, spreadsheetId, sheetId, actualIdColumnIndex);
                        console.log(`[Phase 5: Sheets → Airtable] ✓ Hidden column ${columnLetter} (users won't see record IDs)`);
                    }
                    catch (error) {
        ...
        console.log(`[BidirectionalSync] Airtable → Sheets: ${result.summary.airtableToSheets.added} added, ${result.summary.airtableToSheets.updated} updated`);
        console.log(`[BidirectionalSync] Sheets → Airtable: ${result.summary.sheetsToAirtable.added} added, ${result.summary.sheetsToAirtable.updated} updated`);
        console.log(`[BidirectionalSync] Conflicts: ${result.summary.conflicts.total} resolved`);
...
async function retryWithBackoff(fn, maxRetries, operation, context) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            retryCount = attempt;
            // Classify the error
            const syncError = classifyError(error, context?.phase || 'retry', context?.recordId, context?.rowNumber);
            ...
            console.warn(`[RetryBackoff] ${operation} failed (attempt ${attempt + 1}/${maxRetries + 1}) - ${syncError.type}, ` +
...
async function saveSyncResults(syncConfigId, result) {
    try {
        ...
        console.error(`[SaveSyncResults] Failed to save sync results for config ${syncConfigId}:`, error);
        // Don't throw - this is a non-critical operation
    }
}
// ============================================================================
