...
export class FieldMapperError extends Error {
    ...
    sourceType;
    targetType;
    constructor(message, fieldName, sourceType, targetType) {
        super(message);
 ...
 */
export async function airtableToSheets(value, fieldType, context) {
            ...
            case 'multipleRecordLinks':
                if (Array.isArray(value)) {
                    ...
                    if (context?.accessToken && context?.baseId && context?.airtableField?.options?.linkedTableId) {
                        try {
                            const names = await fetchLinkedRecordNames(value, context.accessToken, context.baseId, context.airtableField.options.linkedTableId);
                            result.value = names.join(', ');
                        }
                        catch (error) {
                            result.warnings?.push(`Failed to fetch linked record names: ${error instanceof Error ? error.message : String(error)}`);
                            // Fallback to IDs
                        ...
                        result.value = value.map((record) => record.id).join(', ');
                        result.warnings?.push('Linked records exported as IDs (no context provided for name lookup)');
                    }
                }
                else {
...
async function fetchLinkedRecordNames(linkedRecords, accessToken, baseId, linkedTableId) {
    ...
    return linkedRecords.map((linkedRecord) => {
        ...
        if (!record)
            return linkedRecord.id; // Fallback to ID if not found
 ...
 */
export async function sheetsToAirtable(value, targetFieldType, context) {
    const result = { value: null, errors: [], warnings: [] };
    try {
        // Handle empty values
        ...
        switch (targetFieldType) {
            ...
            case 'singleSelect':
                ...
                if (context?.airtableField?.options?.choices) {
                    ...
                    const matchingChoice = choices.find((c) => c.name.toLowerCase() === stringValue.toLowerCase());
                    if (matchingChoice) {
                        ...
                        result.errors?.push(`Value "${stringValue}" is not a valid choice for ${fieldName}. Available options: ${choices.map((c) => c.name).join(', ')}. Check your field mappings - this column may be mapped to the wrong Airtable field.`);
                        result.value = null;
                    }
                }
                else {
            ...
            case 'multipleSelects':
                ...
                if (context?.airtableField?.options?.choices) {
                    ...
                    const invalidValues = [];
                    values.forEach((val) => {
                        const matchingChoice = choices.find((c) => c.name.toLowerCase() === val.toLowerCase());
                        if (matchingChoice) {
                    ...
                    if (invalidValues.length > 0) {
                        result.warnings?.push(`Some values are not valid choices: ${invalidValues.join(', ')}. Available: ${choices.map((c) => c.name).join(', ')}`);
                    }
            ...
            case 'multipleRecordLinks':
                ...
                if (context?.accessToken && context?.baseId && context?.airtableField?.options?.linkedTableId) {
                    try {
                        ...
                        const recordIds = await fetchRecordIdsByNames(names, context.accessToken, context.baseId, context.airtableField.options.linkedTableId);
                        result.value = recordIds.map((id) => ({ id }));
                        if (recordIds.length < names.length) {
                            result.warnings?.push(`Some linked records could not be found: ${names.length - recordIds.length} missing`);
                        }
                    }
                    catch (error) {
                        result.errors?.push(`Failed to lookup linked records: ${error instanceof Error ? error.message : String(error)}`);
                        result.value = null;
                    }
                }
                else {
                    result.errors?.push('Cannot convert to linked records without context (accessToken, baseId, linkedTableId required)');
                    result.value = null;
                }
                break;
            // Read-only fields
 ...
 */
export async function airtableRecordToSheetsRow(record, fields, context) {
    const row = [];
    const errors = [];
    const warnings = [];
 ...
 */
export async function sheetsRowToAirtableFields(row, fields, context) {
    const recordFields = {};
    const errors = [];
    const warnings = [];
 ...
 */
export async function sheetsRowToAirtableFieldsWithMapping(row, fields, fieldMappings, idColumnIndex, context) {
    const recordFields = {};
    const errors = [];
    const warnings = [];
    for (const field of fields) {
 ...
 */
export function isWritableFromSheets(fieldType) {
 ...
 */
export function validateFieldCompatibility(airtableField) {
    const issues = [];
