...
import { getSheetData, updateSheetData, hideColumn, columnNumberToLetter, ensureColumnsExist, batchSetDropdownValidations, } from '../google/client';
import { airtableRecordToSheetsRow } from './fieldMapper';
import { resolveAllLinkedRecords } from './linkedRecordResolver';
import { detectDropdownFields, convertToSheetsValidation, logDropdownFields, } from './dropdownFieldDetector';
// ============================================================================
// Main Sync Function
// ============================================================================
/**
...
export async function syncAirtableToSheets(options) {
    ...
    // Set defaults
    const { airtableAccessToken, sheetsAccessToken, baseId, tableId, viewId, // Optional view ID for exact row order
    spreadsheetId, sheetId, fieldMappings, includeHeader = true, resolveLinkedRecords: shouldResolveLinkedRecords = true, idColumnIndex = 0, maxRetries = 3, batchSize = 100, } = options;
    ...
    try {
        ...
        try {
            ...
            airtableRecords = await retryWithBackoff(() => listRecords(airtableAccessToken, baseId, tableId, fetchOptions), maxRetries, 'fetch Airtable records');
        ...
        if (shouldResolveLinkedRecords && airtableRecords.length > 0) {
            ...
            try {
                const resolved = await resolveAllLinkedRecords(airtableAccessToken, baseId, tableId, airtableRecords, { strictMode: false });
                ...
                result.warnings.push(`Failed to resolve linked records: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        // ========================================================================
        ...
        const transformedRowsWithoutIds = []; // Rows without record IDs for writing
        const recordIdMapping = new Map(); // rowIndex -> recordId
        for (let i = 0; i < airtableRecords.length; i++) {
            const record = airtableRecords[i];
            try {
                const { row, errors, warnings } = await airtableRecordToSheetsRow(record, tableFields, {
        ...
        try {
            existingData = await retryWithBackoff(() => getSheetData(sheetsAccessToken, spreadsheetId, sheetId), maxRetries, 'fetch Sheets data');
            console.log(`[AirtableToSheets] Found ${existingData.values?.length || 0} existing rows in Sheets`);
        }
        catch (error) {
        ...
        if (includeHeader && (!existingData.values || existingData.values.length === 0)) {
            ...
            try {
                ...
                await retryWithBackoff(() => updateSheetData(sheetsAccessToken, spreadsheetId, sheetId, 'A1', [headerRow]), maxRetries, 'add header row');
        ...
        if (allRowsInOrder.length > 0) {
            ...
            for (let i = 0; i < writeBatches.length; i++) {
                ...
                try {
                    ...
                    await retryWithBackoff(() => updateSheetData(sheetsAccessToken, spreadsheetId, sheetId, range, batchData), maxRetries, `write rows batch ${i + 1}/${writeBatches.length}`);
                    ...
                    console.log(`[AirtableToSheets] Wrote batch ${i + 1}/${writeBatches.length} (${batch.length} rows)`);
                }
                catch (error) {
                    result.errors.push({
                        type: 'WRITE',
        ...
        if (dropdownFields.length > 0) {
            ...
            try {
                ...
                await batchSetDropdownValidations(sheetsAccessToken, spreadsheetId, sheetId, validations);
        ...
        if (actualIdColumnIndex === FIXED_ID_COLUMN_INDEX && (result.added > 0 || result.updated > 0)) {
            ...
            try {
                ...
                console.log(`[AirtableToSheets] Ensuring column ${columnLetter} exists (need ${requiredColumnCount} columns)...`);
                ...
                for (const [rowIndex, recordId] of recordIdMapping) {
                    ...
                    try {
                        ...
                        result.warnings.push(`Failed to write record ID to ${range}: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
                ...
                console.log(`[AirtableToSheets] ✓ Hidden column ${columnLetter} (users won't see record IDs)`);
            }
            catch (error) {
                // Non-fatal - log warning
                result.warnings.push(`Could not write/hide ID column: ${error instanceof Error ? error.message : String(error)}`);
        ...
        // STEP 10: Finalize
        // ========================================================================
        console.log(`[AirtableToSheets] Sync complete: ${result.added} added, ${result.updated} updated, ${result.deleted} deleted`);
        return finalizeSyncResult(result, startTime);
    }
    catch (error) {
        result.errors.push({
 ...
 */
function calculateRowDiff(newRows, existingRows, idColumnIndex, headerRowOffset, deleteExtra) {
    const diff = {
        toAdd: [],
        toUpdate: [],
        toDelete: [],
...
async function retryWithBackoff(fn, maxRetries, operation) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`[AirtableToSheets] ${operation} failed (attempt ${attempt + 1}/${maxRetries + 1}), ` +
 ...
 */
async function deleteRows(accessToken, spreadsheetId, sheetId, startRow, count) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to delete rows: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`);
    }
}
// ============================================================================
// Exports
...
//# sourceMappingURL=airtableToSheets.js.map