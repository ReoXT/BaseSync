...
export function GoogleSheetsSelector({ value, onChange }) {
    ...
    const [validatedSpreadsheet, setValidatedSpreadsheet] = useState(null);
    ...
    const handleValidateUrl = async () => {
        ...
        try {
            ...
            setValidationError(error instanceof Error ? error.message : "Failed to validate spreadsheet URL");
            setValidatedSpreadsheet(null);
        }
        finally {
    ...
    return (<div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input id="google-sheets-url" type="text" placeholder="Paste your Google Sheets URL here" value={spreadsheetUrl} onChange={(e) => setSpreadsheetUrl(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter") {
                handleValidateUrl();
            }
        }} disabled={isValidating} className="h-11"/>
          <Button onClick={handleValidateUrl} disabled={isValidating || !spreadsheetUrl.trim()} className="h-11 px-6">
            {isValidating ? (<>
      ...
      {validatedSpreadsheet && (<div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedSheetId} onValueChange={handleSheetChange}>
              ...
              <SelectContent>
                {validatedSpreadsheet.sheets.map((sheet) => (<SelectItem key={sheet.sheetId} value={`${sheet.sheetId}|${sheet.title}`}>
