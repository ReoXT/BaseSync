      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-12 md:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-fade-in-delayed">
          ...
          <nav className="md:col-span-1">
            ...
            return (<button key={section.id} className={cn('w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300', 'border backdrop-blur-sm text-left', isActive && 'border-cyan-500/30 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 shadow-lg shadow-cyan-500/10', !isActive && 'border-border bg-card/30 hover:border-cyan-500/20 hover:bg-card/50', section.danger && !isActive && 'border-red-500/20 hover:border-red-500/30 hover:bg-red-500/5', section.danger && isActive && 'border-red-500/30 bg-gradient-to-br from-red-500/10 to-red-600/10 shadow-lg shadow-red-500/10')} onClick={() => setActiveSection(section.id)}>
                    <div className={cn('shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors', isActive && !section.danger && 'bg-cyan-500/20', !isActive && !section.danger && 'bg-muted/50', section.danger && isActive && 'bg-red-500/20', section.danger && !isActive && 'bg-red-500/10')}>
                      <Icon className={cn('h-4 w-4', isActive && !section.danger && 'text-cyan-400', !isActive && !section.danger && 'text-muted-foreground', section.danger && 'text-red-400')}/>
                    </div>
                    <span className={cn('font-medium transition-colors', isActive && !section.danger && 'text-foreground', !isActive && !section.danger && 'text-muted-foreground', section.danger && 'text-red-400')}>
                      {section.label}
                    </span>
                  </button>);
        })}
...
//# sourceMappingURL=AccountSettingsPage.jsx.map