    ...
    <div className="relative min-h-screen w-full bg-background text-foreground overflow-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto">
          ...
          {errorMessage && (
            <div
              className={`max-w-2xl mx-auto mb-8 p-4 rounded-xl border border-red-500/30 bg-red-500/10 backdrop-blur-sm transition-all duration-300 ${
...
function BillingToggle({
  ...
  useEffect(() => {
    const updatePillPosition = () => {
      const activeButton =
        billingPeriod === "monthly" ? monthlyRef.current : annualRef.current;
      if (activeButton) {
        setPillStyle({
...
function PricingCard({
  ...
  const annualSavings = ((tier.monthlyPrice - tier.annualPrice) * 12).toFixed(
    0
  );
...
function TrustSignals({ isVisible }: { isVisible: boolean }) {
  return (
    <div
...
function FAQTeaser({
  ...
  return (
    <div
      className={`mt-20 max-w-4xl mx-auto transition-all duration-700 delay-700 ${
        isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
      }`}
    >
      {/* Section Header */}
      ...
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          <div
            key={index}
            className="p-6 rounded-xl border border-border bg-card/30 backdrop-blur-sm hover:border-cyan-500/30 transition-all duration-300"
          >
