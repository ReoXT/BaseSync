...
export function GoogleSheetsSelector({ value, onChange }: GoogleSheetsSelectorProps) {
  const [selectedSpreadsheetId, setSelectedSpreadsheetId] = useState<string | undefined>(value.spreadsheetId);
  ...
  const handleSheetChange = (sheetIdAndName: string) => {
    ...
    onChange({
      ...
      spreadsheetName: spreadsheets?.find((s: GoogleSpreadsheet) => s.id === selectedSpreadsheetId)?.name,
      sheetId: sheetName,
      sheetName,
    });
  };
