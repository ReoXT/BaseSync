...
import type { AirtableFieldType, AirtableField } from '../airtable/client';
 ...
 */
export function validateForSheets(
  data: Record<string, any>[],
  fieldMappings: Map<string, AirtableField>,
  options: ValidationOptions = {}
): ValidationResult {
  const {
    maxStringLength = SHEETS_MAX_CELL_LENGTH,
    sanitize = true,
    checkRequired = true,
  } = options;
  ...
  for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
    ...
    for (const [fieldName, value] of Object.entries(row)) {
      ...
      const validationResult = validateFieldValue(
        fieldName,
        value,
        { maxStringLength, sanitize, rowIndex: rowIndex + 1 },
        field?.type
      );
 ...
 */
export function validateForAirtable(
  data: Record<string, any>[],
  fieldMappings: Map<string, AirtableField>,
  options: ValidationOptions = {}
): ValidationResult {
  const {
    maxStringLength = AIRTABLE_MAX_STRING_LENGTH,
    sanitize = true,
    checkRequired = true,
  } = options;
    ...
    case 'multipleAttachments':
      ...
      if (Array.isArray(value)) {
        sanitizedValue = value.map(v =>
          typeof v === 'string' && options.sanitize ? sanitizeString(v) : v
        );
 ...
 */
export function validateInBatches(
  data: Record<string, any>[],
  fieldMappings: Map<string, AirtableField>,
  direction: 'airtable-to-sheets' | 'sheets-to-airtable',
  options: ValidationOptions = {},
  ...
  const allSanitizedData: Record<string, any>[] = [];
