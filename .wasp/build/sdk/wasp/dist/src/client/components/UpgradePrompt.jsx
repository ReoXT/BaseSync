...
// Helper Functions
// ============================================================================
function getLimitTypeLabel(limitType) {
    switch (limitType) {
        case "syncs":
...
function getDefaultMessage(limitType, severity, current, limit, currentPlan, requiredPlan) {
...
function getValueProposition(limitType, requiredPlan, priceDifference) {
...
// Component
// ============================================================================
export function UpgradePrompt({ limitType, severity, current, limit, currentPlan, requiredPlan, priceDifference, message, onDismiss, variant = "inline", open, onOpenChange, }) {
    ...
    if (variant === "inline") {
        return (<Alert variant={severity === "blocking" ? "destructive" : "default"} className={cn("relative", severity === "warning" && "border-yellow-500 bg-yellow-50 dark:bg-yellow-950/20")}>
        {severity === "warning" ? (<Zap className="h-4 w-4 text-yellow-600"/>) : (<AlertCircle className="h-4 w-4"/>)}
        <AlertTitle>
        ...
        <AlertDescription className="mt-2">
          ...
          <div className="mt-4 flex gap-3">
            ...
            {onDismiss && severity === "warning" && (<Button size="sm" variant="outline" onClick={handleDismiss}>
                Maybe Later
              </Button>)}
          </div>
        </AlertDescription>
        {onDismiss && (<button onClick={handleDismiss} className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100">
            <X className="h-4 w-4"/>
    ...
    if (variant === "modal") {
        ...
        <DialogContent className={cn(severity === "blocking" && "border-destructive border-2", severity === "warning" && "border-yellow-500 border-2")}>
          <DialogHeader>
            <div className="flex items-center gap-2">
              {severity === "warning" ? (<Zap className="h-5 w-5 text-yellow-600"/>) : (<AlertCircle className="text-destructive h-5 w-5"/>)}
              <DialogTitle>
          ...
          <DialogFooter className="flex gap-3 sm:justify-start">
            ...
            {severity === "warning" && (<Button variant="outline" onClick={handleDismiss}>
                Maybe Later
              </Button>)}
          </DialogFooter>
        </DialogContent>
    ...
    return (<Card className={cn("border-2", severity === "blocking" && "border-destructive", severity === "warning" && "border-yellow-500")}>
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          {onDismiss && (<button onClick={handleDismiss} className="rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100">
              <X className="h-4 w-4"/>
              <span className="sr-only">Dismiss</span>
            </button>)}
        </div>
      ...
      <CardFooter className="flex gap-3">
        ...
        {onDismiss && severity === "warning" && (<Button variant="outline" onClick={handleDismiss}>
            Maybe Later
          </Button>)}
      </CardFooter>
    </Card>);
 ...
 */
export function SyncLimitPrompt(props) {
    return <UpgradePrompt {...props} limitType="syncs"/>;
}
/**
 * Record Limit Prompt
 */
export function RecordLimitPrompt(props) {
    return <UpgradePrompt {...props} limitType="records"/>;
}
/**
 * Sync Frequency Limit Prompt
 */
export function FrequencyLimitPrompt(props) {
