 ...
 */
export function SyncHistory({ syncLogs, isLoading = false, limit = 50, }) {
    ...
    // Loading state
    if (isLoading) {
        return (<div className="relative rounded-2xl border border-cyan-500/20 bg-card/70 backdrop-blur-sm overflow-hidden">
    ...
    return (<div className="relative rounded-2xl border border-cyan-500/20 bg-card/70 backdrop-blur-sm overflow-hidden">
      ...
      <div className="relative border-t border-border/60 divide-y divide-border/60">
        {paginatedLogs.map((log) => {
            ...
            return (<div key={log.id} className="p-4 hover:bg-muted/30 transition-colors">
              <div className="flex items-start justify-between gap-4">
                ...
                {hasErrors && (<button onClick={() => toggleExpanded(log.id)} className="p-1.5 rounded-md hover:bg-muted/50 transition-colors" aria-label={isExpanded ? "Hide error details" : "Show error details"}>
              ...
              {(log.triggeredBy || log.direction) && (<div className="mt-3 text-xs text-muted-foreground flex flex-wrap gap-3">
                  {log.triggeredBy && <span>Triggered by: {log.triggeredBy}</span>}
                  {log.direction && <span>Direction: {log.direction}</span>}
                </div>)}

              {isExpanded && hasErrors && (<div className="mt-4 pt-4 border-t border-border/60 space-y-3">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {errors.map((error, errorIndex) => (<div key={errorIndex} className="rounded-md border border-red-200/70 dark:border-red-500/30 bg-red-50/50 dark:bg-red-950/20 px-3 py-2">
                        {error.record && (<p className="text-[11px] text-red-600/80 dark:text-red-400/80 mb-1">
      ...
      {totalPages > 1 && (<div className="border-t border-border px-4 py-3 flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            Page {currentPage} of {totalPages}
          </div>
          <div className="flex gap-2">
            <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className={cn("px-3 py-1.5 text-xs rounded-md border transition-colors", currentPage === 1
                ? "opacity-50 cursor-not-allowed border-border/60"
                : "border-border hover:bg-muted/50")}>
              Previous
            </button>
            <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className={cn("px-3 py-1.5 text-xs rounded-md border transition-colors", currentPage === totalPages
                ? "opacity-50 cursor-not-allowed border-border/60"
                : "border-border hover:bg-muted/50")}>
              Next
            </button>
...
//# sourceMappingURL=SyncHistory.jsx.map