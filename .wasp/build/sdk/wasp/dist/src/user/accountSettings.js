...
export const updateUsername = async ({ username }, context) => {
    if (!context.user) {
...
export const requestEmailChange = async ({ newEmail }, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Unauthorized');
    }
...
export const confirmEmailChange = async ({ token }, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Unauthorized');
    }
...
export const updateNotificationPreferences = async ({ emailNotifications, syncFailureAlerts, weeklyDigest }, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Unauthorized');
    }
    await context.entities.User.update({
