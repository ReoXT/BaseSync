...
// ============================================================================
export const exportUserData = async (args, context) => {
    if (!context.user) {
    ...
    const allSyncLogs = user.syncConfigs.flatMap(config => config.syncLogs.map(log => ({
        ...log,
...
export const deleteAccount = async ({ confirmationText, password }, context) => {
    if (!context.user) {
        throw new HttpError(401, 'Unauthorized');
    }
    ...
    // Find email identity
    const emailIdentity = user.auth.identities.find((identity) => identity.providerName === 'email');
    if (!emailIdentity) {
    ...
    try {
        ...
        await verifyPassword(emailIdentity.providerUserId, password);
    }
    catch (error) {
