...
import { isApproachingRecordLimit, shouldPauseSyncs, getSyncPauseReason } from '../middleware/usageLimits';
 ...
 */
export const triggerManualSync = async (args, context) => {
    ...
    let syncConfigForLogging = null; // Will be set after we fetch it
    ...
    if (recentSync) {
        ...
        throw new HttpError(409, 'A sync is already in progress for this configuration. Please wait for it to complete.');
    }
    // ==========================================================================
    ...
    let errors = [];
    let warnings = [];
    let caughtError = null; // Track error to rethrow after logging
    try {
        switch (syncConfig.syncDirection) {
            ...
            default: {
                ...
                throw new HttpError(500, `Invalid sync direction: ${syncConfig.syncDirection}. Please contact support.`);
            }
        }
            ...
            data: {
                ...
                status: syncStatus === 'SUCCESS'
                    ...
                    : syncStatus === 'PARTIAL'
                        ? 'PARTIAL'
                        : 'FAILED',
                recordsSynced: syncConfig.syncDirection === 'BIDIRECTIONAL'
    ...
    const result = {
        ...
        message,
        details: {
            added: syncConfig.syncDirection === 'BIDIRECTIONAL'
                ? (syncResult?.summary?.airtableToSheets?.added || 0) +
                    (syncResult?.summary?.sheetsToAirtable?.added || 0)
                : syncResult?.added || 0,
            updated: syncConfig.syncDirection === 'BIDIRECTIONAL'
                ? (syncResult?.summary?.airtableToSheets?.updated || 0) +
                    (syncResult?.summary?.sheetsToAirtable?.updated || 0)
                : syncResult?.updated || 0,
            deleted: syncConfig.syncDirection === 'BIDIRECTIONAL'
        ...
        conflicts: syncConfig.syncDirection === 'BIDIRECTIONAL'
 ...
 */
export const runInitialSync = async (args, context) => {
    const startTime = Date.now();
    const startedAt = new Date();
    console.log('\n' + '='.repeat(80));
    ...
    if (syncConfig.lastSyncAt) {
        const daysSinceLastSync = Math.floor((Date.now() - syncConfig.lastSyncAt.getTime()) / (1000 * 60 * 60 * 24));
        console.warn(`[InitialSync] This sync config was last synced ${daysSinceLastSync} day(s) ago`);
        warnings.push(`This is not a first-time sync. Last sync was ${daysSinceLastSync} day(s) ago.`);
    }
    // Edge case: Sync config is inactive
    if (!syncConfig.isActive) {
    ...
    if (!airtableConnection) {
        ...
        throw new HttpError(400, 'Airtable connection not found. Please connect your Airtable account first.');
    }
    // Edge case: Missing Google Sheets connection
    if (!googleConnection) {
        console.error('[InitialSync] No Google Sheets connection found');
        throw new HttpError(400, 'Google Sheets connection not found. Please connect your Google account first.');
    }
    ...
    if (syncConfig.fieldMappings) {
        try {
            ...
            throw new HttpError(400, `Invalid field mappings configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // ==========================================================================
    ...
    try {
        switch (syncConfig.syncDirection) {
            ...
            case 'BIDIRECTIONAL': {
                ...
                syncStatus = syncResult.status;
                if (args.dryRun) {
                    warnings.push('This was a dry run. No actual changes were made. Run without dryRun flag to apply changes.');
                }
                break;
            }
            // ----------------------------------------------------------------------
