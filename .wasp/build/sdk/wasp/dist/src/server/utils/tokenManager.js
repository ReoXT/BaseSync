...
import { refreshAccessToken as refreshAirtableToken, storeAirtableConnection, } from '../airtable/auth';
import { refreshAccessToken as refreshGoogleToken, storeGoogleSheetsConnection, } from '../google/auth';
 ...
 */
export async function getValidAirtableToken(userId) {
    ...
    if (connection.needsReauth) {
        throw new Error(`Airtable connection needs reauthorization. ${connection.lastRefreshError || 'Please reconnect your Airtable account.'}`);
    }
    ...
    if (isTokenExpiringSoon(connection.tokenExpiry)) {
        ...
        if (!refreshResult.success) {
            throw new Error(`Failed to refresh Airtable token: ${refreshResult.error}. Please reconnect your Airtable account.`);
        }
...
export async function getValidGoogleToken(userId) {
    ...
    // Check if connection is marked as needing reauth
    if (connection.needsReauth) {
        throw new Error(`Google Sheets connection needs reauthorization. ${connection.lastRefreshError || 'Please reconnect your Google account.'}`);
    }
    // Check if token is expired or about to expire
    if (isTokenExpiringSoon(connection.tokenExpiry)) {
        ...
        if (!refreshResult.success) {
            throw new Error(`Failed to refresh Google token: ${refreshResult.error}. Please reconnect your Google account.`);
        }
        return refreshResult.accessToken;
    }
    // Token is still valid, decrypt and return
 ...
 * @returns Valid access token
 */
export async function getValidToken(userId, type) {
    if (type === 'airtable') {
 ...
 */
export async function getConnectionHealth(userId, type) {
    const connection = type === 'airtable'
 ...
 */
export async function hasConnectionsNeedingReauth(userId) {
 ...
 */
export async function forceRefreshToken(userId, type) {
 ...
 * @param type - Connection type
 */
export async function clearNeedsReauth(userId, type) {
