...
import { shouldPauseSyncs, getSyncPauseReason } from '../middleware/usageLimits';
 ...
 */
export const performSync = async (_args, context) => {
    ...
    try {
            ...
            try {
                ...
                try {
                    ...
                    if (config.syncDirection === 'BIDIRECTIONAL') {
                        console.log(`[SyncJob]   Airtable → Sheets: ${syncResult.summary.airtableToSheets.added} added, ${syncResult.summary.airtableToSheets.updated} updated`);
                        console.log(`[SyncJob]   Sheets → Airtable: ${syncResult.summary.sheetsToAirtable.added} added, ${syncResult.summary.sheetsToAirtable.updated} updated`);
                        console.log(`[SyncJob]   Conflicts: ${syncResult.summary.conflicts.total} resolved`);
                    }
                    else {
                        console.log(`[SyncJob]   Records: ${syncResult.added} added, ${syncResult.updated} updated, ${syncResult.deleted} deleted`);
                    }
                    ...
                    try {
                        await sendSyncFailedEmail(config.user, config.name, errorMessage);
                    }
                    catch (emailError) {
                    ...
                    data: {
                        ...
                        status: syncStatus === 'success'
                            ...
                            : syncStatus === 'partial'
                                ? 'PARTIAL'
                                : 'FAILED',
                        recordsSynced: config.syncDirection === 'BIDIRECTIONAL'
                        ...
                        errors: errorMessage
                            ...
                            : syncResult?.errors && syncResult.errors.length > 0
                                ? JSON.stringify(syncResult.errors.slice(0, 10).map((e) => ({
                                    message: e.message,
                                    recordId: e.recordId,
                                    type: e.type,
                                })))
                ...
                try {
                    ...
                    console.error('[SyncJob] Failed to log error:', logError instanceof Error ? logError.message : String(logError));
                }
            }
        }
        // ========================================================================
        ...
        result.results.forEach((r, i) => {
            ...
            console.log(`[SyncJob]   ${i + 1}. ${icon} ${r.configName} - ${r.message}${r.duration ? ` (${r.duration}ms)` : ''}`);
            if (r.error) {
        ...
        // Job-level error
        console.error('\n[SyncJob] ✗ CRITICAL ERROR in sync job:', error instanceof Error ? error.message : String(error));
 ...
 */
function getSyncDirectionDescription(direction) {
    switch (direction) {
        case 'AIRTABLE_TO_SHEETS':
 ...
 */
function truncateError(error, maxLength = 200) {
