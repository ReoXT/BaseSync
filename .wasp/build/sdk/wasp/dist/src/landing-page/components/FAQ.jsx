...
export default function FAQ({ faqs }) {
    ...
    return (<section id="faq" ref={sectionRef} className="relative pt-8 pb-24 md:pt-12 md:pb-32 overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10">
        ...
        {/* Gradient Orb */}
        <div className="absolute -bottom-40 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full bg-cyan-500/5 blur-3xl animate-pulse-slow" aria-hidden="true"/>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        ...
        <div className={`transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <Accordion type="single" collapsible className="w-full space-y-4">
            {faqs.map((faq, index) => (<AccordionItem key={faq.id} value={`faq-${faq.id}`} className="relative rounded-2xl border border-border bg-card/50 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-cyan-500/30 hover:bg-card/70 group" style={{
                animationDelay: `${index * 0.05}s`,
            }}>
                {/* Hover Glow Effect */}
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" style={{
                background: "radial-gradient(circle at 0% 50%, rgba(6, 182, 212, 0.05), transparent 70%)",
            }}/>
                ...
                <div className="relative px-6 py-4">
                  ...
                  <AccordionContent className="text-muted-foreground">
                    <div className="flex flex-col items-start gap-4 pl-11 pt-2">
                      ...
                      {faq.href && (<a href={faq.href} className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 text-sm font-semibold transition-colors duration-200">
                          Learn more
        ...
        <div className={`mt-16 text-center transition-all duration-700 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          ...
          <a href="mailto:support@basesync.example.com" className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-cyan-500/30 bg-cyan-500/5 backdrop-blur-sm text-cyan-400 hover:border-cyan-500 hover:bg-cyan-500/10 transition-all duration-300 hover:scale-105 font-semibold">
