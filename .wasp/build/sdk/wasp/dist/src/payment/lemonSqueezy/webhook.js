...
export const lemonSqueezyWebhook = async (request, response, context) => {
    try {
...
export const lemonSqueezyMiddlewareConfigFn = (middlewareConfig) => {
...
async function handleOrderCreated(data, userId, prismaUserDelegate) {
...
async function handleSubscriptionCreated(data, userId, prismaUserDelegate) {
...
async function handleSubscriptionUpdated(data, userId, prismaUserDelegate) {
    const { customer_id, status, variant_id } = data.attributes;
    const lemonSqueezyId = customer_id.toString();
    const planId = getPlanIdByVariantId(variant_id.toString());
...
async function handleSubscriptionCancelled(data, userId, prismaUserDelegate) {
...
async function handleSubscriptionExpired(data, userId, prismaUserDelegate) {
    const { customer_id } = data.attributes;
    const lemonSqueezyId = customer_id.toString();
    await updateUserLemonSqueezyPaymentDetails({
        lemonSqueezyId,
...
async function fetchUserCustomerPortalUrl({ lemonSqueezyId, }) {
