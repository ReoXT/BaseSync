import { CheckCircle2, XCircle, AlertCircle, RefreshCw, LucideIcon } from "lucide-react";
...
export function ConnectionStatus({
  ...
  const formatLastSynced = (timestamp: string | Date | undefined): string => {
    if (!timestamp) return "";
  ...
  const getStatusText = (): string => {
    ...
    if (isTokenExpired) return "Token expired - please reconnect";
    if (isConnected) {
  ...
  const getButtonConfig = () => {
    if (isTokenExpired) {
      return { text: "Reconnect", variant: "default" as const };
    }
    if (isConnected) {
      return { text: "Reconnect", variant: "outline" as const };
    }
    return { text: `Connect ${serviceName}`, variant: "default" as const };
  };
    ...
    <Card>
      ...
      <CardContent>
        ...
        <CardDescription className="mb-4">
          {statusText}
          {lastSyncedText && isConnected && !isTokenExpired && (
