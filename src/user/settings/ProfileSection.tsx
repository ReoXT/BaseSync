...
export default function ProfileSection() {
  ...
  return (
    <div className="space-y-6">
      ...
      <EmailCard currentEmail={user.email || ''} pendingEmail={(user as any).pendingEmail} />
      <NotificationsCard user={user as any} />
    </div>
  );
}
...
function EmailCard({ currentEmail, pendingEmail }: { currentEmail: string; pendingEmail?: string }) {
  const { toast } = useToast();
  const [isUpdating, setIsUpdating] = useState(false);
...
function NotificationsCard({ user }: { user: any }) {
  const { toast } = useToast();
  ...
  // Auto-save on change
  const handleChange = (field: keyof z.infer<typeof notificationsSchema>, value: boolean) => {
        ...
        <Form {...form}>
          <div className="space-y-3">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex items-center justify-between space-y-0 rounded-xl border border-border bg-background/30 backdrop-blur-sm p-4 transition-all duration-300 hover:border-cyan-500/20">
              ...
              render={({ field }) => (
                <FormItem className="flex items-center justify-between space-y-0 rounded-xl border border-border bg-background/30 backdrop-blur-sm p-4 transition-all duration-300 hover:border-cyan-500/20">
                  ...
                  <FormControl>
                    <Switch
                      checked={field.value}
                      onCheckedChange={(value) => handleChange('syncFailureAlerts', value)}
              ...
              render={({ field }) => (
                <FormItem className="flex items-center justify-between space-y-0 rounded-xl border border-border bg-background/30 backdrop-blur-sm p-4 transition-all duration-300 hover:border-cyan-500/20">
                  ...
                  <FormControl>
                    <Switch
                      checked={field.value}
                      onCheckedChange={(value) => handleChange('weeklyDigest', value)}
                      disabled={!form.watch('emailNotifications')}
                    />
                  </FormControl>
                </FormItem>
